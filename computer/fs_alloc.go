...
func (fs *FileSystem) falloc(inode *inode, newSize uint32) error {
	...
	if blocksNeeded > numOCurrentBlocks {
		...
		num_of_needed_blocks := blocksNeeded - numOCurrentBlocks

		for i := 1; i <= int(num_of_needed_blocks); i++ {
			blkAddressIdx := int(numOCurrentBlocks) + i
			// find free data block
			// allocate the bitmap
			// add the address to blkAdderssIdx
		}
